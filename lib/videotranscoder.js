var Async       = require( __dirname + "/async.js" ).Async,
    ...
    paths       = {
        ...
        "filmstrip": ( function() {
            try {
                var path = fs.realpathSync( __dirname + "/../bin/filmstrip" );
                return path;
            } catch ( e ) {
                return false;
            }
...
function detectCapabilities() {
    ...
    tasker.sync( function() {
        ...
        if ( !paths.filmstrip )
            errors.push( "filmstrip not found!" );
...
function Transcoder() {
    ...
    me.setBinaryName = function( binName ) {
        ...
        if ( [ 'ffmpeg', 'ffmpeg2theora', 'filmstrip' ].indexOf( binName ) == -1 )
    ...
    me.bind( 'stderr', function( buffer ) {
        ...
        if ( 'ffmpeg' == binaryName || 'filmstrip' == binaryName ) {
        
            if ( !!( matches = /(^|[\s]+)frame\=[\s]+([\d]+) /.exec( buffer ) ) )
                me.on( 'frame', integer( matches[2] ) );
            
            if ( !!( matches = /(^|[\s]+)time\=([\d]{2})\:([\d]{2})\:([\d]{2})\.([\d]{2}) /.exec( buffer ) ) ) {
                me.on( 'second', parseFloat( ( ( integer(matches[2]) * 3600 ) + ( integer( matches[3] ) * 60 ) + integer( matches[4] ) ) + '.' + matches[5] ) );
            }
    ...
    */
    ...
    me.run = function( inputFile, outputFile, presetName, callback ) {
        ...
        switch ( true ) {
            ...
            case /\.ogv$/i.test( outputFile ):
                me.setBinaryName( 'ffmpeg2theora' );
                break;
            
            case /\.(zip|gif|png|jpeg|jpg|png)$/i.test( outputFile ):
                me.setBinaryName( 'filmstrip' );
                break;
        ...
        t.sync( function() {
            ...
            switch (true) {
                ...
                case binaryName == 'filmstrip':
                
                    me.on( "status", "2.1 - using filmstrip backend" );
                    
                    // all the first 4 parameters should be setup by
                    // the preset file
                    ...
                    if ( procArgs.length == 6 )
                        this.on( 'success' );
                    else
                        this.on( 'error', "Wrong number of arguments for filmstrip backend" );
                ...
                default:
                    this.on( 'error', "Unknown video transcoder backend: " + binaryName );
                    break;
            }
