...
var apiWorker           = require( __dirname + '/api-worker.js' ).ApiWorker,
    ...
    quotaResponse       = require( __dirname + '/registry.js' ).QuotaCalculator,
    
    integer             = require( __dirname + '/math.js' ).integer,
    
    commandLineArgs     = require( __dirname + '/argv-utils.js' ).customArgs;
...
exports.Api = function() {
    ...
    var thing = require( __dirname + '/thing.js' ).Thing,
        ...
        maxConnections = integer( commandLineArgs.max_connections || 0 );
    
    console.log( "* API: accepting maximum " + maxConnections + " simultaneous uploads." );
    
    me.bind( 'connection-start', function() {
        
        totalConnections++;
        
    } );
    
    me.bind( 'connection-end', function() {
    ...
    // The canHandleNewConnection called after the connection-start event,
    // so we put <= at comparision.
    me.canHandleNewConnection = function() {
    ...
    me.blind( function( info ) {
        ...
        try {
            info.response.sendError( "Unbinded event " + info.eventName );
        } catch ( error ){
    ...
    me.bind( 'worker-subscribe', function( workerInfo ) {
        ...
        try {
            ...
            workerInfo.sendError( e + "" );
        }
        
        
    } );
    
    me.bind( 'quota-stat', function( info ) {
        
        try {
            var storageIp = info.request.socket.remoteAddress,
                storagePort = integer( info.port );
                
            if ( !storagePort )
                throw "Missing .port property from event or bad port value!";
            
            quotaResponse( storageIp + ':' + storagePort, function( err, data ) {
                
                if ( err ) {
                    info.response.sendError( "Failed to calculate quota for " + storageIp + ":" + storagePort + ": " + err );
                } else {
                    info.response.write(JSON.stringify({
                        "ok": true,
                        "data": data
    ...
    me.bind( 'storage-subscribe', function( storageInfo ) {
        ...
        try {
            ...
            storageInfo.response.sendError( e + '' );
    ...
    me.bind( 'set-task-status', function( info ) {
        ...
        if ( !taskId || taskId < 0 || !status ) {
            ...
            info.response.sendError( "Either taskId either status contains illegal values: taskId=" + taskId + ", status=" + status );
        ...
        query.find( { "taskId": taskId }, function( err ) {
            ...
            if ( err ) {
                ...
                info.response.sendError( "Query error: " + err );
                ...
                if ( query.length == 1 ) {
                    ...
                    info.response.sendError( "Query result length is not 1!" );
