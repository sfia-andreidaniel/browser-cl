...
function ApiFileController() {
                            ...
                            if ( !!( matches = /^(video|audio|image|other)\.([a-zA-Z\d_\-]+)\.js$/.exec( files[i] ) ) ) {
                                ...
                                try {
                                    ...
                                    if ( !func ) {
                                        ...
                                        loadedTasks[ matches[1] ] = loadedTasks[ matches[1] ] || [];
                                        
                                        loadedTasks[ matches[1] ].push( matches[2] );
    ...
    tasker.sync( function() {
        ...
        for ( var taskType in loadedTasks ) {
            
            if ( loadedTasks.propertyIsEnumerable( taskType ) && loadedTasks.hasOwnProperty( taskType ) )
                console.log( "* file controller: Loaded " + taskType + " transcoding presets: " + loadedTasks[ taskType ].join( ', ' ) );
    ...
    tasker.run(
        function() {
            ready = true;
            console.log( "* file controller: Initialized" );
            me.on( 'ready' );
        },
        
        function( reason ) {
            console.log( "* file controller: Initialization ERROR: " + ( reason || 'unknown reason' ) );
            error = true;
            me.on( 'error' );
        }
    );
