...
/* Firewall for IPV4 addresses */

exports.firewall4 = function( allowFrom, ipV4Address ) {
    
    allowFrom = allowFrom || [];
    
    var clientIP = ip2long( ipV4Address ),
        matches, classBoundStart, classBoundStop, classScope, classStart;
    
    for ( var i=0,len=allowFrom.length; i<len; i++ ) {
        
        if ( allowFrom[i] == '0.0.0.0' || allowFrom[i] == '0.0.0.0/0' )
            return true;
...
exports.getFirewallList = function( listName ) {
    return firewalls[ listName ]
        ? firewalls[ listName ]
        : ( function() {
            
            try {
                
                var data = JSON.parse( fs.readFileSync( __dirname + '/../conf/firewall.' + listName + '.json' ) + '' );

                if ( data instanceof Array ) {
                    return firewalls[ listName ] = data;
                } else {
                    console.log( "FIREWALL WARNING: failed to parse firewall list " + listName + " as json" );
                    return firewalls[ listName ] = [];
                }
...
exports.reloadFirewall = function() {
    firewalls = {};
}
