...
class WebSocket implements WebSocketObserver
{
    ...
    public function __construct($url, $service, $useHybie = true, $showHeaders = false)
    {
    ...
    public function open()
    {
        ...
        if ( @fwrite($this->socket, $buffer, strlen($buffer)) === FALSE ) {
            throw new Exception("Failed to write server connection headers" );
        }
        ...
        if ( empty( $buffer ) )
            throw new Exception( "Failed to fetch response buffers from server" );
        
        if ( strpos( $buffer, "HTTP/1.1 101 Switching Protocols" ) === FALSE )
            throw new Exception( "WebSocket Server returned a non success response" );
    ...
    protected function buildHeaderArray()
    {
        ...
        $this->headers = array("GET" => "{$this->requestUri} HTTP/1.1", "Connection:" => "Upgrade", "Host:" => "{$this->host}", "Sec-WebSocket-Key:" => "{$this->handshakeChallenge}", "Origin:" => "{$this->origin}", "Sec-WebSocket-Version:" => 13, "Upgrade:" => "websocket", "Sec-WebSocket-Protocol: " => "$this->_service");
    ...
    protected function buildHeaderArrayHixie76()
    {
        ...
        $this->headers = array("GET" => "{$this->requestUri} HTTP/1.1", "Connection:" => "Upgrade", "Host:" => "{$this->host}", "Origin:" => "{$this->origin}", "Sec-WebSocket-Key1:" => "{$this->hixieKey1->key}", "Sec-WebSocket-Key2:" => "{$this->hixieKey2->key}", "Upgrade:" => "websocket", "Sec-WebSocket-Protocol: " => "$this->_service");
